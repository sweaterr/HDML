...
from preprocessing.autoaugment import augmentation_transforms
from preprocessing.autoaugment import policies
...
def _decode_crop_and_flip(image_buffer, bbox, num_channels, use_random_crop,
                          dct_method="", with_drawing_bbox=False):
  ...
  Args:
    ...
    dct_method: An optional `string`. Defaults to `""`.
      ...
      option.)
    with_drawing_bbox: Whether to return drawing bouding boxes.
      If False, raw image's 3-D tensor with bounding box is None.

  Returns:
    (3-D tensor with cropped image, raw image's 3-D tensor with bounding box)
...
def apply_autoaug(image, autoaugment_type):
  if autoaugment_type == b'imagenet':
    selected_policies = policies.imagenet_policies()
    epoch_policy = selected_policies[np.random.choice(len(selected_policies))]
    final_img = augmentation_transforms.apply_policy(epoch_policy, image)
  elif autoaugment_type == b'svhn':
    selected_policies = policies.SVHN_policies()
    epoch_policy = selected_policies[np.random.choice(len(selected_policies))]
    final_img = augmentation_transforms.apply_policy(epoch_policy, image)
  elif autoaugment_type == b'cifar':
    selected_policies = policies.cifar10_policies()
    epoch_policy = selected_policies[np.random.choice(len(selected_policies))]
    final_img = augmentation_transforms.apply_policy(epoch_policy, image)
    final_img = augmentation_transforms.cutout_numpy(final_img)
  elif autoaugment_type == b'good':
    selected_policies = policies.good_policies()
    epoch_policy = selected_policies[np.random.choice(len(selected_policies))]
    final_img = augmentation_transforms.apply_policy(epoch_policy, image)
    final_img = augmentation_transforms.cutout_numpy(final_img)
  else:
...
def preprocess_image(image_buffer, bbox, output_height,
                     ...
                     with_drawing_bbox=False, dct_method="INTEGER_ACCURATE"):
  ...
  Args:
    ...
    dct_method: An optional `string`. Defaults to `""`.
      ...
      jpeg library changes to a version that does not have that specific
      option.)
    autoaugment_type: if 'True', apply auto augmentation.
    with_drawing_bbox: Whether to return drawing bouding boxes.
      If False, raw image's 3-D tensor with bounding box is None.

  Returns:
    (A preprocessed image, A raw image with bounding box)
  """
  if is_training:
    ...
    if autoaugment_type:
      autoaugment_type = tf.convert_to_tensor(autoaugment_type)
      image = tf.py_func(apply_autoaug, [image, autoaugment_type], tf.float32)
  ...
  return _mean_image_subtraction(image, _CHANNEL_MEANS, num_channels), image_with_bbox
